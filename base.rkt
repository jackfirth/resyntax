...
(provide ~replacement
         ...
         (contract-out [refactoring-rule? (-> any/c boolean?)]
...
(module+ private
  (provide (contract-out [refactoring-rule-refactor
...
(define-template-metafunction
 (~replacement stx)
 (syntax-parse stx
   [(_ new-stx #:original orig-syntax) (syntax-property #'new-stx 'replacement-for #'orig-syntax)]
...
(define-object-type refactoring-rule
...
(define (refactoring-rule-refactor rule syntax source)
  ...
  (option-map ((refactoring-rule-transformer rule) prepared-syntax)
              (λ (new-syntax)
                (syntax-replacement #:source source
                                    #:original-syntax syntax
                                    #:new-syntax (rule-introduction-scope new-syntax)
                                    #:introduction-scope rule-introduction-scope))))

(define-syntax-parse-rule (define-refactoring-rule id:id
  ...
  (define id
    (constructor:refactoring-rule #:name 'id
...
(define-syntax-parse-rule (define-definition-context-refactoring-rule
                           id:id
  ...
  (begin
    ...
    (define-syntax-class expression-matching-id
      ...
      (pattern ((~var branching-header branching-form-allowing-internal-definitions-within-clauses)
                ...
                (... ...))
        #:cut
        #:with refactored #'(branching-header.original
                             (... ...)
                             clause-before
                             (... ...)
                             (~replacement [clause-header body.refactored (... ...)]
...
(define-object-type refactoring-suite
                    (rules)
...
(define-syntax-parse-rule (define-refactoring-suite id:id rules:rules-list suites:suites-list)
  (define id (refactoring-suite #:name 'id #:rules (append rules.as-list-expr suites.as-list-expr))))
