...
(provide (contract-out [legacy-struct-migrations refactoring-suite?]))
...
(define-splicing-syntax-class struct-option
                              ...
                              (pattern (~seq (~and keyword:keyword (~not :constructor-name-keyword))
                                             expr:expr ...)
                                #:with (original ...) #'(keyword expr ...)))

(define-refactoring-rule
 define-struct-to-struct
 #:description "The `define-struct` form exists for backwards compatibility, `struct` is preferred."
 #:literals (define-struct)
 (define-struct id:id-maybe-super fields
   option:struct-option ...)
 (struct id.migrated ... fields option.original ... ... #:extra-constructor-name id.make-id))

(define-refactoring-suite legacy-struct-migrations #:rules (define-struct-to-struct))
