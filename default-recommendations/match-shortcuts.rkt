...
(provide (contract-out [match-shortcuts refactoring-suite?]))
...
(define-syntax-class single-clause-match
  ...
  (pattern (match subject
    ...
    #:with (as-definition-context-body ...) #`(~splicing-replacement (definition body ...)
...
(define-definition-context-refactoring-rule
 ...
 #:do [(define pattern-ids (syntax-bound-identifiers (attribute match-expression.match-pattern)))
       (define pattern-ids-in-surrounding-context
         (syntax-bound-identifiers (replace-context (attribute match-expression)
                                                    (attribute match-expression.match-pattern))))
       (define body-ids (syntax-bound-identifiers #'(body-before ... match-expression.subject)))
       (define subject-ids-in-body-context
         (syntax-bound-identifiers (replace-context (first (attribute match-expression.body))
                                                    (attribute match-expression.subject))))]
 ...
 (if (empty? (attribute body-before))
     ...
     #'(~focus-replacement-on (match-expression.as-definition-context-body ...)))
...
(define (match-pattern-add-ellipsis-depth-properties pattern-stx)
  (let loop ([pattern-stx pattern-stx]
             [depth 0])
    (syntax-traverse
     pattern-stx
...
(module+ test
  (test-case "match-pattern-add-ellipsis-depth-properties"
    (define (report-depths stx)
      (syntax->datum
       (syntax-traverse
        (match-pattern-add-ellipsis-depth-properties stx)
        [id:id
...
(define-refactoring-rule
 ...
 (match val-expr
   clause-before ...
   [~and
    [match-pattern:expr (~and option-or-body (~not (=> :id))) ...]
    clause-to-replace]
   clause-after ...)
 ...
 (syntax-find-first (match-pattern-add-ellipsis-depth-properties (attribute match-pattern))
                    ...
                    #:when (equal? (syntax-property (attribute subject) 'match-pattern-ellipsis-depth)
                                   0))
 #:with new-pattern
 ...
 (syntax-traverse (attribute condition)
                  [id:id
                   ...
                   (attribute subject)])
 #:with new-clause
 #'[new-pattern #:when new-condition option-or-body ...]
 (match val-expr
   clause-before ...
   [~replacement
    new-clause
    #:original clause-to-replace]
   clause-after ...))
...
(define-refactoring-rule
 ...
 (for/list ([part (in-list (attribute body-part))])
   (syntax-traverse
    part
    [id:id #:when (free-identifier=? (attribute id) (attribute bound-id)) (attribute subject-var)]))
 (match subject-var
   clause-before ...
   [inner-pattern
    modified-body ...]
...
(define-refactoring-rule
 ...
 (match subject
   clause-before ...
   [~and
    (pattern conditional:conditional-body)
    clause-to-replace]
   clause-after ...+)
 ...
 (match subject
   clause-before ...
   [~@
    . (~splicing-replacement ((pattern #:when
                                conditional.condition
                                conditional.then-expr ...)
...
(define-refactoring-suite match-shortcuts
                          #:rules (match-conditional-to-when predicate-pattern-with-lambda-to-when
                                                             remove-unnecessary-root-and-pattern
                                                             single-clause-match-to-match-define))
