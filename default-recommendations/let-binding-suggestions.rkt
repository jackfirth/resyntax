...
(provide (contract-out [let-binding-suggestions refactoring-suite?]))
...
(define-definition-context-refactoring-rule
 ...
 #:when (for/and ([body-free-id (in-free-id-set (syntax-free-identifiers
                                                 #'(body-before ... nested-expr body-after ...)))])
          (identifier-binding-unchanged-in-context? body-free-id (attribute nested-id)))
 (body-before ...
  (~@ .
      (~focus-replacement-on (~splicing-replacement ((define nested-id nested-expr) (define id expr))
...
(define-definition-context-refactoring-rule
 ...
 (~seq body-before ...
       (begin0 (~and original-let
                     (let ([nested-id:id nested-expr:expr])
                       let-body ...
                       result-expr:expr))
         body-after ...))
 #:when (not (set-member? (syntax-bound-identifiers #'(body-before ... body-after ...)) #'nested-id))
 (body-before ...
  (define nested-id nested-expr)
  let-body ...
  (begin0 (~replacement result-expr #:original original-let)
    body-after ...)))
...
(define-refactoring-rule delete-redundant-let
                         ...
                         #:when (equal? (syntax-e (attribute left-id))
                                        (syntax-e (attribute right-id)))
                         body)

(define-refactoring-suite let-binding-suggestions
                          #:rules (let-to-define begin0-let-to-define-begin0
                                                 define-let-to-double-define
