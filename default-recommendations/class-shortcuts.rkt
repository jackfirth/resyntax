...
(provide (contract-out [class-shortcuts refactoring-suite?]))
...
(define-syntax-class manual-method-chain
  ...
  (pattern (send (~and initial-object (~not _:manual-method-chain)) first-method:id
                 first-method-arg ...)
...
(define-refactoring-rule
 instantiate-to-make-object
 #:description
 "The `instantiate` form is for mixing positional and by-name constructor arguments.\
...
(define-refactoring-suite class-shortcuts
                          #:rules (instantiate-to-make-object instantiate-to-new send-chain-to-send+))
