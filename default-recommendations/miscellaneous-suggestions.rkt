...
(provide (contract-out [miscellaneous-suggestions refactoring-suite?]))
...
(define-refactoring-rule
 ...
 #:description
 "The `else`-`if` branch of this `cond` expression can be collapsed into the `cond`\
 expression."
 #:literals (cond
              else
              if)
 (cond
   clause ...
   [else (if inner-condition inner-then-branch else-branch)])
 (cond
   clause ...
   [inner-condition inner-then-branch]
   [else else-branch]))

(define-refactoring-rule cond-begin-to-cond
                         #:description
                         "The bodies of `cond` clauses are already implicitly wrapped in `begin`."
                         #:literals (cond
                                      begin)
                         (cond
                           clause-before ...
...
(define-refactoring-rule
 and-match-to-match
 #:description
 "This `and` expression can be turned into a clause of the inner `match` expression,\
 reducing nesting."
 #:literals (and match)
 (and and-subject:id
      (match match-subject:id
        match-clause ...))
...
(define-refactoring-suite miscellaneous-suggestions
                          #:rules (and-match-to-match cond-begin-to-cond
                                                      cond-else-if-to-cond
