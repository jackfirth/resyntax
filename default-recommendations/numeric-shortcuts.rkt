...
(provide (contract-out [numeric-shortcuts refactoring-suite?]))
...
(define-refactoring-rule add1-lambda-to-add1
                         #:description
                         "This lambda function is equivalent to the built-in `add1` function."
                         #:literals (+)
...
(define-refactoring-rule
 ...
 (lambda:lambda-by-any-name (x1:id)
                            (~or (> x2:id 0) (< 0 x2:id) (not (<= x2:id 0)) (not (>= 0 x2:id))))
 #:when (free-identifier=? #'x1 #'x2)
 positive?)
...
(define-refactoring-suite numeric-shortcuts
                          #:rules (add1-lambda-to-add1 single-argument-multiply-to-identity
