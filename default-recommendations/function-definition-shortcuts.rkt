...
(provide (contract-out [function-definition-shortcuts refactoring-suite?]))
...
(define/guard (free-identifiers=? ids other-ids)
              ...
              (for/and ([id (in-list id-list)]
                        [other-id (in-list other-id-list)])
...
(define-syntax-class possibly-nested-lambdas
  ...
  (pattern (_:lambda-by-any-name first-argument-list:lambda-header
...
(define/guard (build-function-header original-header formal-lists)
              (guard-match (cons first-formals remaining-formals) formal-lists #:else original-header)
              (guard (or (identifier? original-header) (not (zero-argument-formals? first-formals)))
                     #:else original-header)
              (with-syntax ([formals first-formals])
                (build-function-header #`(#,original-header . formals) remaining-formals)))

(define/guard
 (build-function-body original-header formal-lists innermost-body-forms)
 (guard-match (cons first-formals remaining-formals) formal-lists #:else innermost-body-forms)
 (guard (or (identifier? original-header) (not (zero-argument-formals? first-formals)))
        #:else (build-lambda-expressions formal-lists innermost-body-forms))
 (with-syntax ([formals first-formals])
...
(define-refactoring-rule
 ...
 #:with new-header
 (build-function-header #'id (attribute lambda-form.argument-lists))
 #:with (new-body ...)
...
(define-refactoring-rule
 ...
 #:literals (define)
 (define (~and header (~not :id))
   lambda-form:possibly-nested-lambdas)
 #:when (not (syntax-property this-syntax 'class-body))
 #:do [(define multiline-lambda-header-count
         (count multiline-syntax? (attribute lambda-form.argument-lists)))]
 #:when (< multiline-lambda-header-count 2)
 ...
 #:with new-header
 (build-function-header #'header (attribute lambda-form.argument-lists))
 #:with (new-body ...)
...
(define-refactoring-rule
 ...
 #:when (and (equal? (length (attribute case1-arg)) (length (attribute case2-arg)))
             (equal? (length (attribute case1-arg)) (length (attribute usage1))))
...
(define-refactoring-suite function-definition-shortcuts
                          #:rules (define-lambda-to-define define-lambda-to-curried-define
                                                           define-case-lambda-to-define
                                                           empty-checked-rest-args-to-optional-arg))
