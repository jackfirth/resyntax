...
(provide (contract-out [oneline-syntax? (-> any/c boolean?)] [multiline-syntax? (-> any/c boolean?)]))
...
(define (syntax-line-count stx)
  (transduce (syntax-search stx [:atom]) (mapping syntax-line) (deduplicating) #:into into-count))

(module+ test
  (test-case (name-string oneline-syntax?)
    (check-true (oneline-syntax? #'a))
    (check-true (oneline-syntax? #'(a b c)))
    (check-false (oneline-syntax? #'(a b c)))
    (check-false (oneline-syntax? #'(1 2 3))))

  (test-case (name-string multiline-syntax?)
    (check-false (multiline-syntax? #'a))
    (check-false (multiline-syntax? #'(a b c)))
    (check-true (multiline-syntax? #'(a b c)))
    (check-true (multiline-syntax? #'(1 2 3)))))
