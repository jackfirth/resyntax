...
(provide body-with-refactorable-let-expression
         ...
         (contract-out [identifier-binding-unchanged-in-context? (-> identifier? syntax? boolean?)]
...
(define-splicing-syntax-class
 ...
 (pattern (~seq leading-body ... let-expression:refactorable-let-expression)
...
(define-syntax-class refactorable-let-expression
  ...
  (pattern (header:let-header body:let-body)
    ...
    #:when (not (check-duplicate-identifier (for/list ([id (attribute id)]
                                                       #:unless (unused-id? id))
    ...
    #:with (refactored ...) #`(~splicing-replacement (header.definition ... body.refactored ...)
...
(define-syntax-class binding-clause
  ...
  (pattern [all-ids:id-list rhs:expr]
    #:do
    [(log-resyntax-debug "refactorable-let-expression: checking binding-clause not self-shadowing: ~a"
                         this-syntax)]
    #:when
    (for*/and ([rhs-free-id (in-free-id-set (syntax-free-identifiers (attribute rhs)))]
               [id (in-list (attribute all-ids.id))])
      (log-resyntax-debug "refactorable-let-expression: checking identifier ~a not shadowed by ~a"
                          rhs-free-id
                          id)
      (identifier-would-self-shadow? id rhs-free-id #:full-right-hand-side (attribute rhs)))
    #:cut
    #:with (id ...) (attribute all-ids.id)
    #:with definition (match (attribute id)
                        [(list (? unused-id?) ...) #'rhs]
                        [(list only-id)
                         (syntax-parse (attribute rhs)
                           ...
                           [(_:lambda-by-any-name tail-arg:identifier body ...)
                            #`(define (#,only-id . tail-arg)
                                body ...)]
                           [_
                            #`(define (~replacement #,only-id #:original all-ids)
                                rhs)])]
...
(define-syntax-class id-list
  #:attributes ([id 1])
  (pattern only-id:id
    #:with (id ...) (list (attribute only-id)))
  (pattern (id ...)))
...
(define-syntax-class (binding-group #:nested? [nested #false])
  ...
  (pattern (clause:binding-clause ...)
    ...
    #:cut
    #:with (id ...) #'(clause.id ... ...)
    #:with (definition ...) #`(~splicing-replacement
                               ((~replacement clause.definition #:original clause) ...)
                               #:original #,this-syntax)))
