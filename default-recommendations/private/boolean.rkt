...
(define-syntax-class known-false
  (pattern constant:literal-constant
    #:when (not (attribute constant.value))))

(define-syntax-class known-not-false
  (pattern constant:literal-constant
    #:when (attribute constant.value)))

(define-syntax-class known-true
  (pattern constant:literal-constant
    #:when (equal? (attribute constant.value) #true)))
...
(define-syntax-class likely-boolean-returning
  #:literals (= < > <= >=)
  (pattern id:id
    #:when (string-suffix? (symbol->string (syntax-e #'id)) "?"))
  (pattern (~or = < > <= >=)))
...
(define-syntax-class condition-expression
  ...
  (pattern (not base-condition:expr)
    #:with negated? #true)
  (pattern (~and base-condition:expr (~not (not _)))
    #:with negated? #false))
