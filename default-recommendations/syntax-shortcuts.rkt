...
(provide (contract-out [syntax-shortcuts refactoring-suite?]))
...
(define-refactoring-rule
 ...
 #:when (for/or ([uses-syntax-e? (attribute arg.uses-syntax-e?)])
...
(define-syntax-class format-symbol-argument
  ...
  (pattern (syntax-e inner:format-symbol-argument)
...
(define-refactoring-suite syntax-shortcuts
                          #:rules (format-string-to-format-symbol syntax-e-in-format-id-unnecessary))
