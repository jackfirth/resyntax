...
(provide (contract-out [file-io-suggestions refactoring-suite?]))
...
(define-splicing-syntax-class
 ...
 (pattern (~seq arg:function-call-argument ...)
   #:cut
   #:with (positional ...) (for/list ([expr (in-list (attribute arg.expr))]
                                      [keyword (in-list (attribute arg.keyword))]
                                      #:unless keyword)
                             expr)

   #:attr keyword (for/hasheq ([expr (in-list (attribute arg.expr))]
                               [keyword (in-list (attribute arg.keyword))]
                               #:when keyword)
...
(define-refactoring-rule
 ...
 #:with 'directory
 (or (and (>= (length (attribute args.positional)) 2) (second (attribute args.positional)))
     (hash-ref (attribute args.keyword) '#:copy-from #false))
 #:cut
 #:attr template-arg
 (and (not (empty? (attribute args.positional))) (first (attribute args.positional)))
 #:attr base-dir-arg
 (or (and (>= (length (attribute args.positional)) 3) (third (attribute args.positional)))
     (hash-ref (attribute args.keyword) '#:base-dir #false))
 (make-temporary-directory (~? template-arg) (~? (~@ #:base-dir base-dir-arg))))

(define-refactoring-suite file-io-suggestions #:rules (make-temporary-directory-migration))
