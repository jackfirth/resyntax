...
(provide (contract-out [conditional-shortcuts refactoring-suite?]))
...
(define-refactoring-rule if-x-else-x-to-and
                         ...
                         #:literals (if and)
                         (if x:id
                             (~or (and then-part ...) then-branch:expr)
                             y:id)
...
(define-refactoring-rule if-void-to-when-or-unless
                         ...
                         ((~if conditional.negated? unless when) conditional.condition
                                                                 conditional.body ...))
...
(define-syntax-class when-or-unless-equivalent-cond-expression
  ...
  (pattern (cond
    ...
    #:with negated? #false)
  (pattern (cond
             [(not condition) :block-expression]
             [else (void)])
    #:with negated? #true)
  (pattern (cond
             [condition (void)]
...
(define-syntax-class always-throwing-if-expression
  ...
  #:literals (if)
  (pattern (if condition:condition-expression fail:always-throwing-expression success-expression)
    #:with
    equivalent-guard-expression
    #'((~if condition.negated? unless when) condition.base-condition fail))
  (pattern (if condition:condition-expression success-expression fail:always-throwing-expression)
    #:with
    equivalent-guard-expression
...
(define-definition-context-refactoring-rule
 ...
 (body-before ...
  (~@ . (~focus-replacement-on (~splicing-replacement (throwing-if.equivalent-guard-expression
...
(define-syntax-class always-throwing-cond-expression
  ...
  (pattern (cond
             ...
             [else
              body ...])
    #:with (guard-expression ...) #'(((~if condition.negated? unless when) condition.base-condition
                                                                           fail) ...)))
...
(define-definition-context-refactoring-rule
 ...
 (body-before ...
  (~@ . (~focus-replacement-on
         (~splicing-replacement (throwing-cond.guard-expression ... throwing-cond.body ...)
...
(define-refactoring-rule
 ...
 (cond-id:cond clause ...
               (~and outer-else-clause
                     [else
                      (cond
                        nested-clause ...)]))
 (cond-id clause ... (~@ . (~splicing-replacement (nested-clause ...) #:original outer-else-clause))))
...
(define-syntax-class let-refactorable-cond-clause
  ...
  (pattern [condition:expr leading-body ... let-expr:refactorable-let-expression]
    #:with refactored #`(~replacement [condition
                                       leading-body ...
                                       (~@ . (~focus-replacement-on (let-expr.refactored ...)))]
...
(define-refactoring-rule if-begin-to-cond
                         ...
                         #:when (or (attribute then-expr.uses-begin?)
                                    (attribute else-expr.uses-begin?))
                         (cond
                           [~replacement
                            [condition then-expr.refactored ...]
                            #:original-splice (condition then-expr)]
                           [~replacement
                            [else else-expr.refactored ...]
                            #:original else-expr]))

(define-refactoring-rule
 if-let-to-cond
 #:description "`cond` with internal definitions is preferred over `if` with `let`, to reduce nesting"
 #:literals (if void)
 (if condition
     (~and then-expr:if-arm (~not (void)))
     (~and else-expr:if-arm (~not (void))))
...
(define-refactoring-rule
 and-let-to-cond
 #:description "Using `cond` allows converting `let` to internal definitions, reducing nesting"
 #:literals (and cond)
...
(define-refactoring-rule
 ...
 #:description "Nested `when` expressions can be merged into a single compound `when` expression."
 #:literals (when and
              )
 (when outer-condition:expr
   (when inner-condition:expr
     body:expr ...))
 (when (and outer-condition inner-condition)
   body ...))

(define-refactoring-suite conditional-shortcuts
                          #:rules (always-throwing-cond-to-when always-throwing-if-to-when
                                                                and-let-to-cond
                                                                cond-else-cond-to-cond
                                                                cond-let-to-cond-define
