...
(provide (contract-out
          ...
          [resyntax-analysis-total-sources-modified
           (-> resyntax-analysis? exact-nonnegative-integer?)]
...
(define (resyntax-analysis-write-file-changes! analysis)
  ...
  (for ([source (in-list sources)]
    ...
    (display-to-file (modified-source-contents source)
                     (source-path source)
                     #:mode 'text
                     #:exists 'replace)))
...
(define/guard
 ...
 (guard (string-prefix? full-source "#lang racket")
        #:else (log-resyntax-warning "skipping ~a because it does not start with #lang racket"
...
(define (resyntax-analyze-all-once sources
                                   ...
                                   #:max-modified-lines max-modified-lines)
  (transduce
   (in-hash-entries sources) ; entries with source keys and line range set values
   ...
   (grouping into-list)
   (mapping (λ (e) (refactoring-result-set #:base-source (entry-key e) #:results (entry-value e))))
    ...
    (with-handlers ([exn:fail? (λ (e)
      ...
      (guarded-block
       (guard-match (present replacement)
                    ...
                    #:else absent)
       (guard (syntax-replacement-introduces-incorrect-bindings? replacement)
              #:else (define bad-ids
              ...
              (log-resyntax-warning
               ...
               orig-stx)
              absent)
       (guard (syntax-replacement-preserves-comments? replacement comments)
              #:else
              (log-resyntax-warning
              ...
              absent)
       (present (refactoring-result #:rule-name (object-name rule)
                                    #:message (refactoring-rule-description rule)
                                    #:syntax-replacement replacement)))))

  (falsey->option (for*/first ([rule (in-list rules)]
...
(define (refactor! results)
  (define results-by-path
    (transduce results
               (bisecting (λ (result)
                            (file-source-path (syntax-replacement-source
...
(define (substring-by-range str rng)
  ...
  (define start
    (cond
      ...
      [(equal? (range-bound-type lower-bound) inclusive) (range-bound-endpoint lower-bound)]
      [else (max 0 (sub1 (range-bound-endpoint lower-bound)))]))
...
(module+ test
  (test-case "resyntax-analyze"
    ...
    (check-equal?
     (refactoring-result-string-replacement (first results))
     (string-replacement #:start 13 #:end 28 #:contents (list (inserted-string "(or 1 2 3)"))))))
