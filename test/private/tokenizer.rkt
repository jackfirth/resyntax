...
(provide (contract-out [make-refactoring-test-tokenizer
                        (-> input-port? (-> (or/c position-token? eof-object?)))]))
...
(define-lex-abbrev refactoring-test-literal-integer (repetition 1 +inf.0 numeric))
...
(define (make-refactoring-test-tokenizer port)
  ...
  (define initial-lexer
    (lexer-src-pos [whitespace (return-without-pos (initial-lexer input-port))]
                   [".." (token-DOUBLE-DOT)]
                   ["," (token-COMMA)]
                   ["- "
                    (let ()
                   ...
                   [refactoring-test-literal-string
                    (token-LITERAL-STRING (string->immutable-string
  ...
  (define single-code-line-lexer
    (lexer-src-pos [rest-of-line
                    (let ()
                      (set! active-lexer initial-lexer)
                      (token-CODE-LINE lexeme))]))

  (define multi-code-line-lexer
    (lexer-src-pos [(concatenation (intersection any-char (complement (char-set "-="))) rest-of-line)
                    (token-CODE-LINE lexeme)]
                   [dash-line
                    (let ()
                      (set! active-lexer initial-lexer)
...
(module+ test
  ...
  (test-case "make-refactoring-test-tokenizer"
    ...
    (test-case "empty code blocks"
      ...
      (define tokenizer (make-refactoring-test-tokenizer input))
      (define expected-tokens
        (list (position-token (token-DASH-LINE) (position 1 1 0) (position 5 2 0))
    ...
    (test-case "code lines"
      ...
      (define tokenizer (make-refactoring-test-tokenizer input))
      (define expected-tokens
        (list (position-token (token-SINGLE-DASH) (position 1 1 0) (position 3 1 2))
              (position-token (token-CODE-LINE "#lang racket/base (void)\n")
                              (position 3 1 2)
                              (position 28 2 0))))
      (check-equal? (tokenize-until-eof tokenizer) expected-tokens))
    ...
    (test-case "multiple code lines"
      ...
      (define expected-tokens
        (list (position-token (token-SINGLE-DASH) (position 1 1 0) (position 3 1 2))
              (position-token (token-CODE-LINE "#lang racket/base (f)\n")
                              (position 3 1 2)
                              (position 25 2 0))
              (position-token (token-SINGLE-DASH) (position 25 2 0) (position 27 2 2))
              (position-token (token-CODE-LINE "#lang racket/base (g)\n")
                              (position 27 2 2)
                              (position 49 3 0))))
      (check-equal? (tokenize-until-eof tokenizer) expected-tokens))))
