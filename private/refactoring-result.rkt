...
(provide (contract-out
          ...
          [refactoring-result-set
           (-> #:base-source source?
               #:results (sequence/c refactoring-result?)
               refactoring-result-set?)]
...
(define-record-type refactoring-result-set (base-source results) #:omit-root-binding)
...
(define (refactoring-result-map-commits result-map)
  ...
  (for/fold ([committed-replacements (hash)]
    ...
    (define description (refactoring-result-message (first rule-results)))
