...
(provide (contract-out [run-command (-> string? path-string? ... string?)]))
...
(define (run-command cmd-name . args)
  ...
  (unless cmd-path
    (raise-arguments-error (name run-command)
                           ...
                           "executable"
                           cmd-name))
  (define-values (proc stdout stdin stderr) (apply subprocess #f #f #f cmd-path args))
  ...
  (unless (zero? exit-code)
    (raise-arguments-error (name run-command)
                           ...
                           "command"
                           (string-join (cons cmd-name stringified-args) " ")
                           "exit code"
                           exit-code
                           "stderr"
                           stderr-string))
