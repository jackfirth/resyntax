...
(module doc racket/base
  ...
  (provide (contract-out [evaluator/c contract?]
                         [evaluator-factory? (-> any/c boolean?)]
                         [evaluator-factory-create (-> evaluator-factory? evaluator/c)]
                         [make-module-sharing-evaluator-factory
                          (->* ()
                               (#:private (listof module-path?) #:public (listof module-path?))
                               evaluator-factory?)]))
  ...
  (define (make-module-sharing-evaluator-factory #:public [public-modules empty]
                                                 #:private [private-modules empty])
    (define base-factory (make-base-eval-factory (append private-modules public-modules)))
    (define (factory)
