...
(struct hash-value-syntax (key-datum) #:transparent #:sealed)
...
(define (syntax-path-neighbors? leading-path trailing-path)
  (and (nonempty-syntax-path? leading-path)
       ...
       (syntax-path-element-neighbors? (syntax-path-last-element leading-path)
                                       (syntax-path-last-element trailing-path))))
...
(define (syntax-ref init-stx path)
  (define result
    (for/fold ([stx init-stx]) ([element (in-treelist (syntax-path-elements path))])
      (define unwrapped
...
(define (syntax-set init-stx path new-subform)
  (let loop ([stx init-stx]
             [elements (syntax-path-elements path)])
    (guarded-block
...
(define syntax-path-element<=>
  (matching-comparator [(? exact-nonnegative-integer? i) #:compare i]
...
(module+ test
  (test-case "syntax-path-element<=>"
    ...
    (define sorted (sort unsorted (λ (a b) (compare-infix syntax-path-element<=> a < b))))
...
(define syntax-path<=>
  (comparator-map (lexicographic-comparator syntax-path-element<=>)
                  syntax-path-elements
                  #:name 'syntax-path<=>))

(define (possibly-improper-list-of-minimum-size? v size)
  (or (zero? size) (and (pair? v) (possibly-improper-list-of-minimum-size? (cdr v) (sub1 size)))))
