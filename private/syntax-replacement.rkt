...
(provide (contract-out
          ...
          [syntax-replacement-dropped-comment-locations
           (-> syntax-replacement? range-set? range-set?)]))
...
(define-record-type syntax-replacement (original-syntax new-syntax source introduction-scope))
...
(define (syntax-replacement-render replacement #:format? [format? #true])
  ...
  (define/guard
   (pieces stx #:focused? [focused? #false])
   (guard (or focused? (not (syntax-property stx 'focus-replacement-on)))
          #:else (log-resyntax-debug "focusing in on ~a" stx)
          (list (focus (pieces stx #:focused? #true))))
   (guard (not (syntax-completely-original? stx))
          #:else (log-resyntax-debug "copying original syntax ~a" stx)
   ...
   (syntax-parse stx
     ...
     [(subform ...)
      ...
      (append (list (inserted-string opener))
  ...
  (define focused-start
    (+ start
       (for/sum ([piece (in-list contents-with-possible-focus)] #:break (focus? piece))
                (replacement-string-span piece))))
  (define focused-end
    (- end
       (for/sum ([piece (in-list (reverse contents-with-possible-focus))] #:break (focus? piece))
                (replacement-string-span piece))))
  (define raw-contents
    (append-map (λ (piece)
                  (if (focus? piece)
                      (focus-contents piece)
                      (list piece)))
                contents-with-possible-focus))
  (define unformatted (string-replacement #:start start #:end end #:contents raw-contents))
  (when (log-level? resyntax-logger 'debug)
    (define message (string-indent (pretty-format unformatted) #:amount 2))
  ...
  (cond
    ...
    [else
     (define normalized
       (if has-focus?
           (string-replacement-normalize unformatted
...
(define (string-replacement-format replacement original)
  ...
  (log-resyntax-debug "about to format unformatted code at indentation ~a:\n~a"
                      initial-columns
                      (string-indent changed-code-substring #:amount 2))
...
(module+ test
  (test-case (name-string syntax-replacement-render)
    ...
    (define new-stx
      (flip (syntax-parse (flip orig-stx)
              #:datum-literals (+)
