...
(define (append-all-id-maps id-maps)
  (for/fold ([combined (hash)]) ([map id-maps])
...
(define-syntax-class (fully-expanded-top-level-form [phase 0])
  ...
  (pattern (begin
             (~var body (fully-expanded-top-level-form phase)) ...)
    #:attr bound-ids-by-phase (append-all-id-maps (attribute body.bound-ids-by-phase))
    #:attr used-ids-by-phase (append-all-id-maps (attribute body.used-ids-by-phase)))

  (pattern (begin-for-syntax
             (~var body (fully-expanded-top-level-form (add1 phase))) ...)
    #:attr bound-ids-by-phase (append-all-id-maps (attribute body.bound-ids-by-phase))
    #:attr used-ids-by-phase (append-all-id-maps (attribute body.used-ids-by-phase))))
...
(define-syntax-class (fully-expanded-module-level-form phase)
  ...
  (pattern (begin-for-syntax
             (~var body (fully-expanded-module-level-form (add1 phase))) ...)
    #:attr bound-ids-by-phase (append-all-id-maps (attribute body.bound-ids-by-phase))
    #:attr used-ids-by-phase (append-all-id-maps (attribute body.used-ids-by-phase)))
...
(define-syntax-class (fully-expanded-general-top-level-form phase)
  ...
  (pattern (define-values (id:id ...) (~var rhs (fully-expanded-expression phase)))
    #:attr bound-ids-by-phase (append-all-id-maps (list (hash phase (list->treelist (attribute id)))
...
(define-syntax-class (fully-expanded-expression phase)
  ...
  (pattern (#%plain-lambda (~var formals (fully-expanded-formals phase))
    ...
    #:attr bound-ids-by-phase (append-all-id-maps (cons (attribute formals.bound-ids-by-phase)
  ...
  (pattern (case-lambda
             [(~var formals (fully-expanded-formals phase))
              (~var body (fully-expanded-expression phase)) ...+] ...)
    #:attr bound-ids-by-phase (append-all-id-maps (append* (attribute formals.bound-ids-by-phase)
                                                           (attribute body.bound-ids-by-phase)))
  ...
  (pattern (if (~var condition (fully-expanded-expression phase))
    ...
    #:attr bound-ids-by-phase (append-all-id-maps (list (attribute condition.bound-ids-by-phase)
                                                        (attribute true-branch.bound-ids-by-phase)
                                                        (attribute false-branch.bound-ids-by-phase)))
    #:attr used-ids-by-phase (append-all-id-maps (list (attribute condition.used-ids-by-phase)
  ...
  (pattern (begin0 (~var result (fully-expanded-expression phase))
    ...
    #:attr bound-ids-by-phase (append-all-id-maps (cons (attribute result.bound-ids-by-phase)
                                                        (attribute post-body.bound-ids-by-phase)))
    #:attr used-ids-by-phase (append-all-id-maps (cons (attribute result.used-ids-by-phase)
  ...
  (pattern (let-values ([(id:id ...) (~var rhs (fully-expanded-expression phase))] ...)
    ...
    #:attr bound-ids-by-phase (append-all-id-maps (append (list (hash phase immediately-bound-ids))
                                                          (attribute rhs.bound-ids-by-phase)
                                                          (attribute body.bound-ids-by-phase)))
    #:attr used-ids-by-phase (append-all-id-maps (append (attribute rhs.used-ids-by-phase)
  ...
  (pattern (set! id:id (~var rhs (fully-expanded-expression phase)))
    ...
    #:attr used-ids-by-phase (append-all-id-maps (list (hash phase (treelist (attribute id)))
  ...
  (pattern (with-continuation-mark (~var key (fully-expanded-expression phase))
                                   (~var value (fully-expanded-expression phase))
                                   (~var result (fully-expanded-expression phase)))
    #:attr bound-ids-by-phase (append-all-id-maps (list (attribute key.bound-ids-by-phase)
                                                        (attribute value.bound-ids-by-phase)
                                                        (attribute result.bound-ids-by-phase)))
    #:attr used-ids-by-phase (append-all-id-maps (list (attribute key.used-ids-by-phase)
...
(define-syntax-class (fully-expanded-formals phase)
  ...
  (pattern (id:id ...+ . rest-id:id)
    #:attr bound-ids-by-phase (hash phase
                                    (treelist-add (list->treelist (attribute id))
                                                  (attribute rest-id)))
    #:attr used-ids-by-phase (hash))
...
(define (identifier-binding-table bound-ids-by-phase used-ids-by-phase)
  (for/hash ([phase (in-set (set-union (hash-key-set bound-ids-by-phase)
                                       (hash-key-set used-ids-by-phase)))])
