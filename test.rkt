...
(define-syntax (statement stx)
  (syntax-parse stx
    ...
    [(statement statement-id:id . tail)
     ...
     (unless transformer
       (raise-syntax-error #false "unbound identifier" this-syntax #'statement-id))
     (unless (statement-transformer? transformer)
       (raise-syntax-error #false "not defined as a statement" this-syntax #'statement-id))
...
(module reader racket/base
  ...
  (provide (contract-out [read procedure?] [read-syntax procedure?]))
  ...
  (define (read-syntax source-name in)
    ...
    (define cleaned-parse-tree
      (syntax-traverse
       parse-tree
       #:datum-literals
       (standalone-code-block starting-code-block middle-code-block ending-code-block)
